...
var _chunk4KPMO4YRjs = require('./chunk-4KPMO4YR.js');


var _chunk26CIYNM3js = require('./chunk-26CIYNM3.js');
...
// node/cli.ts
var import_fast_deep_equal = _chunkYUG22S6Wjs.__toESM.call(void 0, _chunk26CIYNM3js.require_fast_deep_equal.call(void 0, ));
...
_fs.injectPreparserExtensionLoader.call(void 0, async (headmatter, filepath) => {
  ...
  const roots = (
    /* uniq */
    [
      _chunk4KPMO4YRjs.getUserRoot.call(void 0, {}).userRoot,
      ..._chunk4KPMO4YRjs.getAddonRoots.call(void 0, addons, ""),
      _chunk4KPMO4YRjs.getClientRoot.call(void 0, )
    ]
  );
  const mergeArrays = (a, b) => a.concat(b);
  return await _chunkKKGXM3XLjs.loadSetups.call(void 0, roots, "preparser.ts", { filepath, headmatter }, [], false, mergeArrays);
});
var cli = _yargs2.default.scriptName("slidev").usage("$0 [args]").version(_chunk4KPMO4YRjs.version).strict().showHelpOnFail(false).alias("h", "help").alias("v", "version");
cli.command(
  "* [entry]",
  ...
  async ({ entry, theme, port: userPort, open, log, remote, tunnel, force, inspect }) => {
    ...
    async function initServer() {
      if (server)
        await server.close();
      const options = await _chunk4KPMO4YRjs.resolveOptions.call(void 0, { entry, remote, theme, inspect }, "dev");
      port = userPort || await findFreePort(3030);
      server = await _chunk4KPMO4YRjs.createServer.call(void 0, 
        options,
        {
          server: {
            port,
        ...
        },
        {
          onDataReload(newData, data) {
            if (!theme && _chunk4KPMO4YRjs.resolveThemeName.call(void 0, newData.config.theme) !== _chunk4KPMO4YRjs.resolveThemeName.call(void 0, data.config.theme)) {
...
cli.command(
  ...
  async (args) => {
    ...
    const { build } = await Promise.resolve().then(() => require("./build-K54USNDZ.js"));
    for (const entryFile of entry) {
      const options = await _chunk4KPMO4YRjs.resolveOptions.call(void 0, { entry: entryFile, theme, inspect }, "build");
...
cli.command(
  ...
  async ({ entry }) => {
    for (const entryFile of entry) {
      const data = await _chunk4KPMO4YRjs.parser.load(entryFile);
      _chunk4KPMO4YRjs.parser.prettify(data);
      await _chunk4KPMO4YRjs.parser.save(data);
    }
  }
);
cli.command(
  ...
  (command) => {
    return command.command(
      ...
      async ({ entry, dir, theme: themeInput }) => {
        const data = await _chunk4KPMO4YRjs.parser.load(entry);
        const theme = _chunk4KPMO4YRjs.resolveThemeName.call(void 0, themeInput || data.config.theme);
        if (theme === "none") {
          console.error('Cannot eject theme "none"');
          _process2.default.exit(1);
        }
        if (_chunk4KPMO4YRjs.isPath.call(void 0, theme)) {
          console.error("Theme is already ejected");
          _process2.default.exit(1);
        }
        const roots = _chunk4KPMO4YRjs.getThemeRoots.call(void 0, theme, entry);
        if (!roots.length) {
...
cli.command(
  ...
  async (args) => {
    ...
    for (const entryFile of entry) {
      const options = await _chunk4KPMO4YRjs.resolveOptions.call(void 0, { entry: entryFile, theme }, "export");
      const server = await _chunk4KPMO4YRjs.createServer.call(void 0, 
        options,
        {
          server: { port },
          clearScreen: false
      ...
      printInfo(options);
      _chunk4KPMO4YRjs.parser.filterDisabled(options.data);
...
cli.command(
  ...
  async ({
    ...
    const port = await findFreePort(12445);
    for (const entryFile of entry) {
      const options = await _chunk4KPMO4YRjs.resolveOptions.call(void 0, { entry: entryFile }, "export");
      const server = await _chunk4KPMO4YRjs.createServer.call(void 0, 
        options,
        {
          server: { port },
...
function printInfo(options, port, remote, tunnelUrl) {
  ...
  console.log(`${_kolorist.bold.call(void 0, "  Slidev")}  ${_kolorist.blue.call(void 0, `v${_chunk4KPMO4YRjs.version}`)} ${_isinstalledglobally2.default ? _kolorist.yellow.call(void 0, "(global)") : ""}`);
  console.log();
